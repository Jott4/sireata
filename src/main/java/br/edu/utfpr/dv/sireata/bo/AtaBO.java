...
public class AtaBO {
    ...
    public AtaReport gerarAtaReport(int idAta) throws Exception {
        try {
            ...
            report.setNumero(df.format(ata.getNumero()) + "/" + DateUtils.getYear(ata.getData()));
            ...
            texto = this.getDataExtenso(ata.getData()) + ", no " + StringEscapeUtils.escapeHtml4(ata.getLocalCompleto()) +
                    ...
                    " de " + DateUtils.getYear(ata.getData()) + " do(a) " +
            ...
            for (int i = 1; i <= ata.getPauta().size(); i++) {
                texto += "(" + i + ") " + StringEscapeUtils.escapeHtml4(ata.getPauta().get(i - 1).getTitulo()) + (i == ata.getPauta().size() ? "." : "; ");
            }

            texto += "</b> " + ata.getConsideracoesIniciais() + " ";

            for (int i = 1; i <= ata.getPauta().size(); i++) {
                texto += "<b>(" + i + ") " + StringEscapeUtils.escapeHtml4(ata.getPauta().get(i - 1).getTitulo()) +
